...
def parse_args():
    ...
    parser.add_argument('--data_dir',type=str, default='AwA2', help='[AwA1 / AwA2 / CUB / / SUN]')
...
if __name__ == '__main__':
    ...
    for epo in range(args.epoch):
        ...
        # learner_fea, learner_lab = learner_fea[:4000], learner_lab[:4000]
        ...
        for i in range(args.inner_loop):
            ...
            dists = dists.t()
            '''
                将欧式距离的计算转到cpu上，显存占用显著下降, 8000->1000。
            '''
            # dists = euclidean_distance(learner_pro_img.cpu(), learner_fea.cpu()) # (40, bs)
            # dists = dists.cuda().t() # (bs, 40)
            ...
            import pynvml
            pynvml.nvmlInit()
            handle = pynvml.nvmlDeviceGetHandleByIndex(3) # 0表示显卡标号
            meminfo = pynvml.nvmlDeviceGetMemoryInfo(handle)
            print('epoch:{} inner loop:{} mem used:{}'.format(epo+1, i+1, meminfo.used/1024**2))  #已用显存大小
            del dists
            c_loss.backward()
            c_optimizer.step()
            '''
